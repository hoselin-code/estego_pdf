...
#
# msg may be str or bytes (e.g. an image envelope)
#
# Returns a list n[]:
...
def encode_msg(msg,key,nbits):
	# Normalise to bytes for hashing and binary processing
	...
	if isinstance(key, str):
		...
		key_bytes = key
	return [digest_to_nums(msg_bytes,nbits),msg_to_nums(msg_bytes,nbits),digest_to_nums(key_bytes,nbits)]
...
def pad_str(msg,nbits):
	...
	return [pad_binstr(b,nbits) for b in split_len(str_to_binstr(msg,nbits),nbits)]
