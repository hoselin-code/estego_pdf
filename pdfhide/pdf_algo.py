...
class PDF_stego:
	...
	#
	# data may be either a str (text message) or bytes (binary payload, e.g. an
	# image envelope produced by image_handler.wrap()).
	#
	# Returns the number of embedded numerals constituting the data
	def embed(self,data,passkey,norandom=False):
		# Normalise data to bytes so encode_msg can handle both text and binary
